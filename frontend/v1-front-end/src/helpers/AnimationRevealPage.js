...
function AnimatedSlideInComponent({ direction = "left", offset = 30, children }) {
  ...
  if (direction === "left") x.initial = "-100%";  // adjusted from -150% to -100%
  else x.initial = "100%";  // adjusted from 150% to 100%

  return (
    <div ref={ref} style={{ overflow: "hidden" }}>  {/* Added overflow hidden */}
      <motion.section
        initial={{ x: x.initial }}
        animate={{
          x: inView && x.target,
